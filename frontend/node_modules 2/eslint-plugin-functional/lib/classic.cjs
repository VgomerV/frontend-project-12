...
function isChainExpression(node) {
    return node.type === utils.AST_NODE_TYPES.ChainExpression;
}
...
function isLabeledStatement(node) {
    return node.type === utils.AST_NODE_TYPES.LabeledStatement;
}
...
function isTryStatement(node) {
    return node.type === utils.AST_NODE_TYPES.TryStatement;
}
...
function isTSNonNullExpression(node) {
    return node.type === utils.AST_NODE_TYPES.TSNonNullExpression;
}
...
function createRuleUsingFunction(name, meta, defaultOptions, createFunction) {
    const ruleCreator = eslintUtils.RuleCreator((ruleName) => `https://github.com/eslint-functional/eslint-plugin-functional/blob/v6.6.3/docs/rules/${ruleName}.md`);
    return ruleCreator({
        name,
        meta: meta,
        defaultOptions,
...
var classic = {
    ...
    configs: {
        ...
        recommended: { plugins: [ruleNameScope], rules: recommended$1 },
